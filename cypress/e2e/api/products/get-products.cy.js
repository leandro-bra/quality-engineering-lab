...
describe("GET /products", () => {
	...
	context("Cenário de erro", () => {
		it("Deve retornar erro 404 ao buscar produto por id inexistente", () => {
			const idProdutoInexistente = 9999;
			cy.request({
				method: "GET",
				url: `/products/${idProdutoInexistente}`,
				failOnStatusCode: false,
			}).then((response) => {
				expect(response.status).to.eq(404);
				expect(response.body).to.have.property(
					"message",
					`Product with id '${idProdutoInexistente}' not found`,
				);
			});
		});
	});
